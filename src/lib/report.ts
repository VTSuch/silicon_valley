...
export async function pipelineReport(): Promise<string> {
  ...
  const [candidatesRes, eventsRes] = await Promise.all([
  ...
  const journeys = buildJourneys(candidates, events)
  ...
  for (const stage of REPORT_STAGES) {
    ...
    for (const j of here) {
      ...
      group.names.push(j.candidate.full_name)
      byRole.set(id, group)
    }
    ...
    const lines = [...byRole.values()].flatMap((group) => [
      ...
      ...group.names.map((name) => `${INDENT}◦ ${escapeHtml(name)}`),
    ])
